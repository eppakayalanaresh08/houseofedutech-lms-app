...
export default function ViewerScreen() {
  ...
  const html = buildCourseHtml(course, headers);
    ...
    <Screen>
      ...
      {hasError ? (
        <View className="gap-3 rounded-2xl bg-[#FCE8E8] px-4 py-4">
          <Text className="text-sm text-danger">The embedded lesson failed to load. Try re-opening the viewer.</Text>
          <Button
            ...
            onPress={async () => {
              ...
              setHasError(false);
              setMessage('Stored a fallback progress update while the WebView is unavailable.');
            }}
          />
        </View>
      ) : (
        <View className="flex-1 overflow-hidden rounded-[28px] border border-line">
          <WebView
            ...
            onMessage={async (event) => {
              try {
                const payload = JSON.parse(event.nativeEvent.data) as { type?: string; courseId?: string };

                if (payload.type === 'chapter_complete' && payload.courseId) {
                  await enroll(payload.courseId);
                  await advanceProgress(payload.courseId, 18);
                  setMessage('Chapter completion received from the embedded lesson and progress was updated.');
                  return;
                }
              } catch {
